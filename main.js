...
(function() {
  ...
  // ===== ОТКРЫТИЕ КАМЕРЫ =====
  // ВАЖНО: <video> создаётся динамически при каждом открытии камеры.
  async function openCamera() {
    ...
    try {
      ...
      // Удаляем старый <video>, если вдруг остался
      ...
      // Вставляем первым элементом, чтобы он был под кнопками
  ...
  // ВАЖНО: полностью удаляем <video> из DOM, чтобы не было замороженного кадра
  async function closeCameraAndReturnToSite() {
    // 1. Мгновенно скрываем камеру
    ...
    // 3. Полностью удаляем <video> из DOM
    const video = document.getElementById('cameraVideo');
    if (video) {
      try {
        video.pause();
    ...
    // 4. Сразу показываем главную страницу
    document.body.classList.remove('fade-out');
