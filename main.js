// main.js — камера, лента контейнеров, геолокация, IndexedDB
(function() {
  'use strict';
  ...
  function getPhotosGroupedByDays() {
    return new Promise(function(resolve) {
      ...
      request.onsuccess = function() {
        ...
        const sortedDays = Object.keys(groups).sort(function(a, b) {
          return b.localeCompare(a);
        });

        const result = sortedDays.map(function(dayKey) {
          const dayPhotos = groups[dayKey].sort(function(a, b) {
  ...
  function createDayContainer(day) {
    ...
    dateEl.textContent = formatDayTitle(day.lastTimestamp);
  ...
  async function openCamera() {
    ...
    try {
      ...
      video.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:cover;background:#000;backface-visibility:hidden;-webkit-backface-visibility:hidden;transform:translateZ(0);';
  ...
  shutterBtn.addEventListener('click', async function() {
    ...
    // Пересобираем ленту контейнеров
    ...
    setTimeout(function() {
      ...
      setTimeout(async function() {
        ...
        if (!isNaN(photoId)) {
          ...
          // Если открыт экран дня и фото больше нет — закрываем
          if (currentDayKey && dayGallery.children.length === 0) {
            closeDayScreen();
          }
        }
      }, 500);
  ...
  saveBtn.addEventListener('click', async function() {
    ...
    try {
      ...
      await savePhotoWithCoords(dataURL, location);
  ...
  deleteDayYesBtn.addEventListener('click', async function() {
    ...
    try {
      ...
      request.onsuccess = async function() {
        ...
        photos.forEach(function(photo) {
          const photoDayKey = getDayKey(new Date(photo.createdAt));
          if (photoDayKey === dayKeyToDelete) {
            store2.delete(photo.id);
          }
        });

        tx2.oncomplete = async function() {
          await rebuildContainersGallery();
        };
      };
    } catch (e) {
  ...
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
      ...
      if (deleteDayOverlay.classList.contains('active')) {
        closeDeleteDayModal();
        return;
      }
