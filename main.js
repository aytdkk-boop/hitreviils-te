...
(function() {
  ...
  const cameraFlashGuard = document.getElementById('cameraFlashGuard');
  ...
  // ===== РАЗРЕШЕНИЕ ГЕОЛОКАЦИИ (резервный путь) =====
  ...
  // ===== АВТООПРЕДЕЛЕНИЕ ГЕОЛОКАЦИИ ПРИ ЗАХОДЕ =====
  ...
  shutterBtn.addEventListener('click', async function() {
    ...
    cameraFlashGuard.classList.add('active');
    ...
    cameraFlashGuard.classList.remove('active');

    if (window.autoSaveEnabled) {
      await savePhotoWithCoords(photoDataURL, currentLocation);
    }

    await closeCameraAndShowPage(photoDataURL);
    isCapturing = false;
  ...
  async function closeCameraAndShowPage(photoDataURL) {
  ...
  // Колбэк, вызываемый после активации или загрузки — из modal.js
