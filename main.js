...
(function() {
  ...
  async function openCamera() {
    ...
    try {
      ...
      // Удаляем старый <video>, если остался
  ...
  async function closeCameraAndReturnToSite() {
    ...
    // Удаляем <video> из DOM, чтобы не было замороженного кадра
    const video = document.getElementById('cameraVideo');
    if (video) {
      try {
        video.pause();
  ...
  function openActionModal(photoElement) {
    ...
    // Показываем кнопку "Сохранить" только если автосохранение ВЫКЛЮЧЕНО
    if (window.autoSaveEnabled) {
