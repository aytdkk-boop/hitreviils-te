// settings.js — настройки, ID, контейнер с ключом, автосохранение, тема
(function() {
  'use strict';
  ...
  try {
    ...
    const savedTheme = localStorage.getItem('hitrevil_theme');
    if (savedTheme === 'light') {
      themeEnabled = true;
  ...
  themeToggle.addEventListener('change', function() {
    ...
    if (themeEnabled) {
      document.body.classList.add('light-theme');
      try {
        localStorage.setItem('hitrevil_theme', 'light');
      } catch (e) {}
    } else {
      document.body.classList.remove('light-theme');
      try {
        localStorage.setItem('hitrevil_theme', 'dark');
      } catch (e) {}
    }
  });
