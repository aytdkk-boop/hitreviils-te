...
(function() {
  ...
  keyInput.addEventListener('input', async function() {
    ...
    if (result.valid) {
      ...
      // Ключ уже активирован — пускаем сразу
      let currentSiteId = result.site_id;
      if (!currentSiteId) {
        try {
      ...
      try {
        ...
        if (currentSiteId) {
          localStorage.setItem('hitrevil_site_id', currentSiteId);
        }
  ...
  function startKeyCheck() {
    ...
    keyCheckInterval = setInterval(async function() {
      // Если приложение в фоне — не проверяем
      ...
      try {
        ...
        console.log('Ключ сброшен или истёк, перезагрузка...');
        try {
          localStorage.removeItem('hitrevil_activated');
          localStorage.removeItem('hitrevil_key');
          localStorage.removeItem('hitrevil_key_expires');
